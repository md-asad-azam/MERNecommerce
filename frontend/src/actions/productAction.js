...
import {
    ...
    NEW_REVIEW_SUCCESS,
    NEW_REVIEW_FAIL,
    ALL_PRODUCT_ADMIN_REQUEST,
    ALL_PRODUCT_ADMIN_SUCCESS,
    ALL_PRODUCT_ADMIN_FAIL,
    NEW_PRODUCT_REQUEST,
    NEW_PRODUCT_SUCCESS,
    NEW_PRODUCT_FAIL,
...
export const getAdminProducts = () => async (dispatch) => {
    ...
    try {
        dispatch({ type: ALL_PRODUCT_ADMIN_REQUEST })

        const { data } = await axios.get(`/api/v1/admin/products`)

        dispatch({
            type: ALL_PRODUCT_ADMIN_SUCCESS,
            payload: data.products
        })

    } catch (error) {
        dispatch({
            type: ALL_PRODUCT_ADMIN_FAIL,
            payload: error.response.data.message
        })
    }
}
